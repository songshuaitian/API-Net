...
def paired_random_crop(img_gts, img_lqs, img_segs,gt_patch_size, scale, gt_path=None):
    ...
    if input_type == 'Tensor':
        img_segs = [v[:, :, top:top + lq_patch_size, left:left + lq_patch_size] for v in img_segs]
    else:
        img_segs = [v[top:top + lq_patch_size, left:left + lq_patch_size, ...] for v in img_segs]
    if len(img_gts) == 1:
        img_gts = img_gts[0]
