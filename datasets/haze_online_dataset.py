...
class HazeOnlineDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.seg_folder = opt['dataroot_seg']
        self.gt_paths = make_dataset(self.gt_folder)
        self.seg_paths = make_dataset(self.seg_folder)
    ...
    def __getitem__(self, index):
        ...
        # print(1111111111111,img_seg.shape)
        ...
        if self.opt['phase'] == 'train':
            ...
            if self.opt['use_resize_crop']:
                ...
                img_seg = random_resize(img_seg, resize_factor)
                t = random_resize(t, resize_factor)

                # random crop
                img_gt, img_lq,img_seg = paired_random_crop(img_gt, img_lq, img_seg,gt_size, input_gt_size // input_lq_size,
                                               gt_path)

            # flip, rotation
            # print(22222222222222,img_seg.shape)
            img_gt, img_lq,img_seg = augment([img_gt, img_lq,img_seg], self.opt['use_flip'],
                                     self.opt['use_rot'])
            # print(3333333333333333333, img_seg.shape)
        ...
        if self.opt['phase'] != 'train':
            ...
            if crop_eval_size:
                ...
                scale = input_gt_size // input_lq_size
                img_gt, img_lq,img_seg = paired_random_crop(img_gt, img_lq,img_seg, crop_eval_size, input_gt_size // input_lq_size,
                                               gt_path)


        # TODO: color space transform
        # BGR to RGB, HWC to CHW, numpy to tensor
        # print(444444444444444444444444,img_seg.shape)
        img_gt, img_lq,img_seg = img2tensor([img_gt, img_lq,img_seg], bgr2rgb=True, float32=True)
...
@DATASET_REGISTRY.register()
class HazeOnlineDataset_rtts(data.Dataset):
    """Paired image dataset for image restoration.
    ...
    def __init__(self, opt):
        super(HazeOnlineDataset_rtts, self).__init__()
        self.opt = opt
        # file client (io backend)
        self.file_client = None
        self.io_backend_opt = opt['io_backend']
    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train':
            ...
            if self.opt['use_resize_crop']:
                ...
                # random crop
                img_gt, img_lq, img_seg = paired_random_crop(img_gt, img_lq, img_seg, gt_size,
        ...
        if self.opt['phase'] != 'train':
            ...
            if crop_eval_size:
                ...
                input_lq_size = img_lq.shape[0]
                scale = input_gt_size // input_lq_size
                img_gt, img_lq, img_seg = paired_random_crop(img_gt, img_lq, img_seg, crop_eval_size,
                                                             input_gt_size // input_lq_size,
                                                             gt_path)
